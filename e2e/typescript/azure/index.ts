...
const clusterName = "pulumi-castai-e2e-test-aks-ts";

// Create a resource group for all resources
const resourceGroup = new azure.resources.ResourceGroup("castai-e2e-rg-ts", {
    location: location,
});

// Create a virtual network
const vnet = new azure.network.VirtualNetwork("castai-e2e-vnet-ts", {
...
const subnet = new azure.network.Subnet("castai-e2e-subnet-ts", {
    resourceGroupName: resourceGroup.name,
...
const managedCluster = new azure.containerservice.ManagedCluster("castai-e2e-aks-ts", {
    resourceGroupName: resourceGroup.name,
    location: resourceGroup.location,
    agentPoolProfiles: [{
        count: 1,
...
const k8sProvider = new k8s.Provider("k8s-provider-ts", {
    kubeconfig: kubeconfig,
});

// Create a service principal for CAST AI
const castaiSp = new azure.authorization.Application("castai-e2e-sp-ts", {});

const castaiServicePrincipal = new azure.authorization.ServicePrincipal("castai-e2e-service-principal-ts", {
    applicationId: castaiSp.applicationId,
});

const castaiPassword = new azure.authorization.ServicePrincipalPassword("castai-e2e-sp-password-ts", {
    servicePrincipalId: castaiServicePrincipal.id,
    endDate: "2099-01-01T00:00:00Z",
});

// Grant CAST AI permissions to the cluster
const roleAssignment = new azure.authorization.RoleAssignment("castai-e2e-role-assignment-ts", {
...
const castaiAksCluster = new castai.AksCluster("castai-e2e-aks-ts", {
    resourceGroupName: resourceGroup.name,
