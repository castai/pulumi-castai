...
const clusterName = "pulumi-castai-e2e-test-eks-ts";

// Create a VPC for our cluster
const vpc = new aws.ec2.Vpc("castai-e2e-vpc-ts", {
...
for (let i = 0; i < 2; i++) {
    const publicSubnet = new aws.ec2.Subnet(`castai-e2e-public-subnet-ts-${i}`, {
        vpcId: vpc.id,
    ...
    const privateSubnet = new aws.ec2.Subnet(`castai-e2e-private-subnet-ts-${i}`, {
        vpcId: vpc.id,
...
const internetGateway = new aws.ec2.InternetGateway("castai-e2e-igw-ts", {
    vpcId: vpc.id,
    tags: {
...
const publicRouteTable = new aws.ec2.RouteTable("castai-e2e-public-rt-ts", {
    vpcId: vpc.id,
    routes: [
        {
            cidrBlock: "0.0.0.0/0",
...
for (let i = 0; i < 2; i++) {
    new aws.ec2.RouteTableAssociation(`castai-e2e-public-rta-ts-${i}`, {
...
const castaiPolicy = new aws.iam.Policy("castai-e2e-policy-ts", {
...
const castaiRole = new aws.iam.Role("castai-e2e-role-ts", {
        ...
        Statement: [
            {
                ...
                Condition: {
                    StringEquals: {
                        "sts:ExternalId": "castai-eks-test-ts",
                    },
                },
            },
        ],
...
const castaiPolicyAttachment = new aws.iam.RolePolicyAttachment("castai-e2e-policy-attachment-ts", {
    role: castaiRole.name,
    policyArn: castaiPolicy.arn,
});

// Create the CAST AI EKS integration
const castaiEksCluster = new castai.EksCluster("castai-e2e-eks-ts", {
