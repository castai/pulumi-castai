...
func TestPulumiAWSOnboarding(t *testing.T) {
	...
	// Create stack with existing project
	stack, err := auto.SelectStackInlineSource(ctx, stackName, projectName, workDir, nil)

	// If stack doesn't exist, create it
	if err != nil {
		stack, err = auto.NewStackInlineSource(ctx, stackName, projectName, workDir, nil)
	...
	// To be implemented - use CAST AI API directly when needed
